...
<!-- <div class='head'></div> -->
<div class='outer' v-loadmore="getmoredata">
        <ul class='products' >
        <li class='products__item' v-for = "one in item" :key='one._id' @click='testscroll'>
                 <img :src="one.imgUrl"/>
...
export default {
    ...
    directives:{
        loadmore:{
            ...
            mounted(el,binding){
                ...
                let inheight;
                let oldScrollTop;
                const scrollel = el.children[0];
                const cb = binding.value;
                el.addEventListener('touchstart',()=>{
                    outheight = scrollel.clientHeight;
                    inheight = el.clientHeight;

                })
                el.addEventListener('touchmove',()=>{
                     loadmore()
                    
                    // console.log(el.scrollTop,scrollel.clientHeight)
                })
                ...
                const loadmore = ()=>{
                     if(inheight+el.scrollTop>=outheight){
                        cb()
                    }
                }
                const moveEnd = () => {
    ...
    const getData = async function(timeout=false){
        ...
        return {item,props,testscroll,getData,getmoredata}
}
    }
